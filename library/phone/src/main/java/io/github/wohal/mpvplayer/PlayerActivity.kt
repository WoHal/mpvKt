...
import live.mehiz.mpvkt.ui.player.PIP_INTENT_ACTION
...
abstract class PlayerActivity : BasePlayerActivity() {
  ...
  override fun onUserLeaveHint() {
    if (isPipSupported && playerViewModel.paused == false && playerViewModel.playerPreferences.automaticallyEnterPip.get()) {
  ...
  override fun onStart() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
      window.attributes.layoutInDisplayCutoutMode = if (playerViewModel.playerPreferences.drawOverDisplayCutout.get()) {
    ...
    if (playerViewModel.playerPreferences.rememberBrightness.get()) {
      playerViewModel.playerPreferences.defaultBrightness.get().let {
  ...
  companion object {
    ...
    private const val RESULT_INTENT = "io.github.wohal.mpvplayer.PlayerActivity.result"
  }
}
